...
class UserServiceTest { // ? TDD => Red, Green, Refactor
    ...
    void testCreateUser_shouldThrowExceptionWhenUserAlreadyExists() {
        ...
        // When & Then
        UserAlreadyExistsException exception = assertThrows(UserAlreadyExistsException.class, () -> userService.createUser(requestDto));
    ...
    @Test
    void testCreateUser_shouldSanitizeInputBeforeSaving() {
        // Given
        ...
        sanitizedUser.setEmail("safe@example.com");

        given(userMapper.toEntity(any())).willReturn(sanitizedUser);
        given(userRepository.save(any())).willReturn(sanitizedUser);

        CreateUserResponseDto responseDto = new CreateUserResponseDto(1L, "alert('XSS')", "safe@example.com");
        given(userMapper.toCreateDto(any())).willReturn(responseDto);
        ...
        // Then
        assertThat(response).isNotNull();
        assertThat(response.username()).isEqualTo("alert('XSS')");
        assertThat(response.email()).isEqualTo("safe@example.com");
    ...
    @Test
    void testGetUserById_shouldReturnEncodedUsername() {
        // Given
        ...
        GetUsersResponseDto encodedResponse = new GetUsersResponseDto(1L, "&lt;b&gt;bold&lt;/b&gt;", "user@example.com");

        given(userRepository.findById(1L)).willReturn(Optional.of(user));
        given(userMapper.toGetDto(user)).willReturn(encodedResponse);

        // When
        GetUsersResponseDto response = userService.getUserById(1L);

        // Then
        assertThat(response).isNotNull();
        assertThat(response.username()).isEqualTo("&lt;b&gt;bold&lt;/b&gt;");
